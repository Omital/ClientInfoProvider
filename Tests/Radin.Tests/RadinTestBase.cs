...
namespace Radin.Tests
{
    public abstract class RadinTestBase : AbpIntegratedTestBase<RadinTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected RadinTestBase()
        {
        ...
        protected void UsingDbContext(Action<RadinDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<RadinDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<RadinDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<RadinDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<RadinDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<RadinDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<RadinDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<RadinDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<RadinDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<RadinDbContext, Task<T>> func)
        {
            T result;
